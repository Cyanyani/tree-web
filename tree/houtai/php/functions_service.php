...
function left_nav($form){
	$nav0_first="<p class='p2'>服务动态</p>";
	$nav0="<p class='p3'><a  href='service.php?form=service-anli'>服务案例<span>></span></a></p>";
	//--------------------------------------------------------------------------------------
	switch($form){
		case "service-anli":$nav0="<p class='p3'><a class='active' href='service.php?form=service-anli'>服务案例<span>></span></a></p>";break;
		default:$nav0="<p class='p3'><a class='active' href='service.php?form=service-anli'>服务案例<span>></span></a></p>";break;
	}
	$nav_all=$nav0_first;
	$nav_all.=$nav0;
	/*$nav_all.=$nav6;*/
...
function nei_nav($form){
	switch($form){
		...
		default:$nav="<span>服务案例</span>";break;
	}
	echo $nav;		
}
//-------------------------------------------------------------------
...
function  fengye($form,$page,$module){
	...
	while($row=mysql_fetch_array($dao)){
		if($star<=$i&&$i<$end){
			if($star<$end){
				++$star;
					$li.="<li><a href='service_detail.php?form=".$_GET["form"]."&id=";
					$li.=$row["id"];
					$li.="' target='_blank'>";
					//只有当有图片时才输出
					if($row["picture"]){
					...
					$li.="</a>";
					//-----------------------------------------------------
					$li.="<a class='tit' href='service_detail.php?form=".$_GET["form"]."&id=".$row["id"];
					$li.="&page=".$_GET["page"];
					$li.="' target='_blank'>";
					//判断tite字符串长度
					...
					$li.="</a>";
					//-----------------------------------------------------
					$li.="<a class='cont' href='service_detail.php?form=".$_GET["form"]."&id=".$row["id"];
					$li.="&page=".$_GET["page"];
					$li.="' target='_blank'>";
					//判断content字符串长度
