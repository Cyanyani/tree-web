...
function titile($form){
	...
	switch($form){
			case "general-introduce":$div.="<span>资源介绍</span></div>";break;
			case "general-natural":$div.="<span>自然资源</span></div>";break;
			case "general-production":$div.="<span>生产概况</span></div>";break;
			
			case "general-jishu":$div.="<span>技术支持</span></div>";break;
			case "general-liangpin":$div.="<span>良种介绍</span></div>";break;
			
			default:$div.="<span>资源介绍</span></div>";break;
		}
			...
			if($star<=$i&&$i<$end){
				if($star<$end){
					++$star;
						...
						//只有当有图片时才输出
						if($row["picture"]){
							$li.="<a href='general_detail.php?form=".$form."&id=";
							$li.=$row["id"];
							$li.="' target='_blank'>";
							$li.="<img src='";
							$li.=$row["picture"];
						...
						//-----------------------------------------------------
						$li.="<a class='tit' href='general_detail.php?form=".$form."&id=".$row["id"];
						$li.="&page=".$page;
						$li.="' target='_blank'>";
						//判断tite字符串长度
						...
						$li.="</a>";
						//-----------------------------------------------------
						$li.="<a class='cont' href='general_detail.php?form=".$form."&id=".$row["id"];
						$li.="&page=".$page;
						$li.="' target='_blank'>";
						//判断content字符串长度
						if($row["picture"]){
